...
impl Game for Legacy {
    ...
    fn create(
        ...
        (WalletGame::default()).create(initialization, duration)
    }
}
...
fn version_one_save_and_peer_are_rejected() {
    ...
    assert!(
        matches!(synctick::replay(WalletGame::default(), &path, &synctick::SessionControl::default()),
        Err(SessionError::Compatibility(message)) if message == "different game version")
    );
    let mut load = ServerConfig::new(Initialization {
        seed: 42,
        balances: vec![10, 20],
    });
    load.port = 0;
    ...
    assert!(matches!(synctick::host(WalletGame::default(), load),
        Err(SessionError::Compatibility(message)) if message == "different game version"));
    ...
    config.expected_clients = 1;
    let mut server = synctick::dedicated_server(WalletGame::default(), config).unwrap();
    wait(|| {
        server.poll();
